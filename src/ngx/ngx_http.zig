...
test "http" {
    ...
    try expectEqual(@sizeOf(ngx_http_request_t), 1384);
    try expectEqual(@offsetOf(ngx_http_request_t, "connection"), 8);
    try expectEqual(@offsetOf(ngx_http_request_t, "cleanup"), 1168);
    try expectEqual(@offsetOf(ngx_http_request_t, "flags0"), 1184);
    try expectEqual(@offsetOf(ngx_http_request_t, "flags1"), 1192);
    try expectEqual(@offsetOf(ngx_http_request_t, "state"), 1200);
    try expectEqual(@offsetOf(ngx_http_request_t, "host_end"), 1368);
    try expectEqual(@offsetOf(ngx_http_request_t, "flags2"), 1376);
    ...
    try expectEqual(@sizeOf(ngx_http_upstream_t), 1088);
