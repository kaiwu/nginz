...
test "http" {
    ...
    try expectEqual(@sizeOf(ngx_http_core_loc_conf_t), 728);
    try expectEqual(@sizeOf(ngx_http_headers_in_t), 368);
    ...
    try expectEqual(@sizeOf(ngx_http_request_t), 1376);
    try expectEqual(@offsetOf(ngx_http_request_t, "connection"), 8);
    try expectEqual(@offsetOf(ngx_http_request_t, "cleanup"), 1168);
    try expectEqual(@offsetOf(ngx_http_request_t, "flags0"), 1176);
    try expectEqual(@offsetOf(ngx_http_request_t, "flags1"), 1184);
    try expectEqual(@offsetOf(ngx_http_request_t, "state"), 1192);
    try expectEqual(@offsetOf(ngx_http_request_t, "host_end"), 1360);
    try expectEqual(@offsetOf(ngx_http_request_t, "flags2"), 1368);
    ...
    try expectEqual(@sizeOf(ngx_http_upstream_conf_t), 544);
