...
const canary_loc_conf = extern struct {
    ...
    cookie_value: ngx_str_t,
    canary_upstream: ngx_str_t,
};
...
export fn ngx_http_canary_handler(
    r: [*c]ngx_http_request_t,
) callconv(.c) ngx_int_t {
    _ = r;
    // TODO: Implement canary routing logic
    // 1. Check header/cookie match
    // 2. Or apply percentage-based routing
    // 3. Set upstream variable for proxy_pass
    return NGX_DECLINED;
}

fn postconfiguration(cf: [*c]ngx_conf_t) callconv(.c) ngx_int_t {
    _ = cf;
    // TODO: Register rewrite phase handler
    return NGX_OK;
}
...
export const ngx_http_canary_commands = [_]ngx_command_t{
    ...
    ngx_command_t{
        .name = ngx_string("canary_upstream"),
        .type = conf.NGX_HTTP_LOC_CONF | conf.NGX_CONF_TAKE1,
        .set = conf.ngx_conf_set_str_slot,
        .conf = conf.NGX_HTTP_LOC_CONF_OFFSET,
        .offset = 0,
